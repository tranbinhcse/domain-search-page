<template>
      ...
      <Box>
        <div class="md:flex md:items-center md:justify-between md:space-x-5">
          ...
          <div class="pt-1.5">
              ...
              <p class="text-sm text-gray-500">Hạn thanh toán <time class="font-medium">{{ invoice.paybefore }}</time></p>
          </div>
          </div>
      ...
      <div class="flex gap-4">
        ...
        <Box v-if="invoice.status == 'Unpaid'" class="flex-auto">
            <div class=" px-4 py-6 sm:px-6 lg:px-8" v-if="invoice.status == 'Unpaid'">
                  ...
                  <p class="mt-2 flex items-baseline gap-x-2">
                    <a-select :style="{'min-width':'220px'}" placeholder="Please select ..." v-model="paymentMethod" @change="changePaymentMethod" >
                        <a-option v-for="(name, id) in paymentMethods" :value="id" :key="id" :label="name" />
                    </a-select> 
                  </p>
              </div>
              <div class=" px-4 py-6 sm:px-6 lg:px-8">
  ...
  const { invoice, transactions, invoiceBody, loading, paymentMethod  } = storeToRefs(invoiceDetailStore)
