    ...
    <form v-else class="mt-12 lg:grid lg:grid-cols-12 lg:items-start lg:gap-x-12 xl:gap-x-16">
      <section aria-labelledby="cart-heading" class="lg:col-span-7">
          ...
          >
                ...
                <div class="ml-4 flex flex-1 flex-col justify-between sm:ml-6">
                  <div class="relative pr-9">
                    <div class="w-full">
                      ...
                      <select
                        class="border-gray-300 border p-2 rounded"
                        @change="handleChangePeriod(product, $event)"
                        :name="`quantity-${productIdx}`"
                        v-model="product.years"
                      >
                        <option
                          :value="cycles.value"
                          v-for="cycles in product?.cycles?.items"
                          :key="cycles.value"
                        >
                          {{ cycles.formatted }}
                        </option>
            ...
            <div v-else class="flex pb-3 sm:pb-6">
              <div class="flex-shrink-0">
                <img
                  src="@/assets/products/img_cart-domains-default.svg"
                  class="h-24 w-24 rounded-md object-cover object-center sm:h-38 sm:w-38"
                />
              </div>
              <div class="ml-4 flex flex-1 flex-col justify-between sm:ml-6">
                <h3 class="text-xl">
        ...
        <div v-else class="mt-8">
          <a-button v-if="!quoteLoading" type="primary" @click="goCheckout()" class="w-full"
            >Tiếp tục</a-button
          >
        </div>
      </section>
      ...
      >
        ...
        <template v-else>
          <div v-if="cartQuote">
            ...
            <div
              class="flex justify-between text-base font-medium text-gray-900"
              v-if="cartQuote.summary.discount"
            >
              <p>Giảm giá</p>
            ...
            <div class="mt-5">
              ...
              <ul>
                <li
                  ...
                  v-for="recurring in cartQuote.summary.recurring"
                  :key="recurring.title"
                >
                  <p>{{ recurring.title }}</p>
