<template>
  <div class="h-full">
        ...
        <div class="box-search-domain text-center">
          <div class="form-search-domain">
            <div class="find-domain-header">
              ...
              <p class="p-2">
                Nhập từ khoá của bạn để đăng ký và bảo vệ thương hiệu của bạn trên internet!
              </p>
            </div>
          ...
          <div>
            <p class="text-gray font-italic text-sm p-4">
              * Bảo vệ quyền riêng tư WHOIS miễn phí được bao gồm trong mọi tên miền nếu đuôi tên
              miền được phép ẩn thông tin.
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="max-w-7xl m-auto" id="searchResults" ref="searchResultsRef">
      ...
      <Button
        v-if="searching"
        btnClass="bg-white text-gray w-full h-20 flex  items-center justify-center"
        isLoading
        textLoading="Chúng tôi đang tìm kiếm tên miền phù hợp với bạn..."
      />
      <div class="mt-10 m-auto text-center" v-if="!searching & (domains.length > 0)">
        <Button
          @click="searchDomains(true)"
          iconPosition="right"
              ...
              <div class="flex flex-1 flex-col p-8">
                ...
                <dl class="mt-1 flex flex-grow flex-col justify-between">
                  ...
                  <dd class="mt-3">
                    ...
                    <span
                      class="inline-flex items-center rounded-full bg-green-50 px-2 py-1 text-sm font-bold text-green-700 ring-1 ring-inset ring-green-600/20 ml-3"
                      >{{ tld.save }}</span
                    >
                  </dd>
                  <div class="mt-4">
                    <span class="font-bold text-3xl text-red-500">{{ tld.price }}</span
                    ><span>đ/năm</span>
                  </div>
                </dl>
              </div>
...
const scrollTosearchResults = () => {
  if (searchResultsRef.value) {
    const topPosition =
      searchResultsRef.value.getBoundingClientRect().top + window.pageYOffset - 100
