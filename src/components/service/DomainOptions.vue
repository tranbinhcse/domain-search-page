  ...
  <div class="mb-8">
    <a-tabs>
      <a-tab-pane key="register" v-if="options.register == '1'">
        <template #title> Đăng ký tên miền </template>
        <div>
          <div
            class="search-form-wrapper w-full bg-gradient-to-r from-primary from-10% via-green-600 to-green-300 rounded-t-md p-6"
          >
            <div class="search-form-wrapper relative">
              <a-input
                ...
                v-model="searchKey"
                name="domain"
                placeholder="Nhập từ khoá muốn tìm"
                class="w-full text-3xl h-[45px]"
              />
              <a-button
                type="primary"
                :loading="searching"
                @click="handleSearchSubmit"
                class="absolute top-1/2 -translate-y-1/2 right-0 h-[45px]"
              >
      ...
      <a-tab-pane key="owndomain" v-if="options.owndomain">
        <template #title> Đã có tên miền </template>
        <div>
          <div
            class="search-form-wrapper w-full bg-gradient-to-r from-primary from-10% via-green-600 to-green-300 rounded-t-md p-6"
          >
      ...
      <a-tab-pane key="m" v-if="options.subdomain">
        <template #title> Tên miền phụ </template>
        <div>
          <div
            class="search-form-wrapper w-full bg-gradient-to-r from-primary from-10% via-green-600 to-green-300 rounded-t-md p-6"
          >
            <div class="search-form-wrapper relative">
              ...
              <a-button
                ...
                :loading="searching"
                @click="handleSearchSubmit"
                class="absolute top-1/2 -translate-y-1/2 right-[10px] h-[45px]"
              >
                Tìm kiếm
                <template #icon>
                  <IconSearch />
    ...
    >
      <form
        ...
        action="/"
        method="get"
        class="relative focus-visible:outline-none focus:outline-none focus:outline-none"
      >
        <a-input
          type="text"
          append="heroicons-outline:search"
          v-model="searchKey"
          name="domain"
          placeholder="Nhập tên miền của bạn"
          size="leading-[32px]"
          class="bg-white rounded-md text-black w-full forcus-visible:outline-none focus:outline-none"
        />
        <Button
          :isLoading="searching"
          type="submit"
          ...
          class="flex-auto absolute right-0 top-[50%] h-[50px] -translate-y-[50%] rounded-tr-md rounded-br-md focus:outline-none"
        />
      </form>
    </div>
  </div>
