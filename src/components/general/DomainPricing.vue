          ...
          <table class="min-w-full divide-y divide-gray-300">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr v-for="tld in tlds" :key="tld.tld">
                  ...
                  <div class="text-primary flex">
                    ...
                    <Tooltip bg="light" size="lg" position="bottom" class="cursor-pointer">
                      ...
                      <template v-slot:content>
                        <p v-if="tld.periods[0].feeRegOrigin">
                          Lệ phí đăng ký: {{ $currency(tld.periods[0].feeRegOrigin) }}
                        </p>
                        <p v-if="tld.periods[0].feeRenOrigin">
                          Phí duy trì: {{ $currency(tld.periods[0].feeRenOrigin) }}
                        </p>
                        <p v-if="tld.periods[0].feeManOrigin">
                          Phí dịch vụ quản trị: {{ $currency(tld.periods[0].feeManOrigin) }}
                        </p>
                        <p v-if="tld.periods[0].vatFee">
                          Thuế VAT: {{ $currency(tld.periods[0].vatFee) }}
                        </p>
                      </template>
                    </Tooltip>
                  </div>
                  <div class="mt-1 text-gray-500 line-through" v-if="tld.periods[0].before > 0">
                    {{ $currency(tld.periods[0].before) }}
                  </div>
                </td>
                <td class="whitespace-nowrap px-3 py-5 text-sm text-gray-500">
                  <div class="text-primary flex">
                ...
                <td class="whitespace-nowrap px-3 py-5 text-sm text-gray-500">
                  {{ $currency(tld.periods[0].transfer) }}
                </td>
