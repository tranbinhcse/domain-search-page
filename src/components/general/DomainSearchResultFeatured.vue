<template>
  <div v-for="domain in domains" :key="domain.id">
    <div
      v-if="!domain.error && domain.isFeatured"
      class="rounded-md px-6 bg-white mt-8"
      :class="[
        domain.isFeatured ? 'featured py-10 bg-green-50  py-6  shadow' : '',
        domain.status == 'inCheck'
          ? 'check-domain'
          : domain.status == 'ok'
            ? 'found-domain'
            : 'not-found-domain'
    ...
    >
      <div class="flex items-center flex-wrap py-2">
        <div class="name flex-1 flex items-center">
          ...
          <div class="">
            <span class="domain font-bold w-full text-lg" v-if="domain.domain">{{
              domain.domain
            }}</span>
            <span class="domain font-bold w-full text-lg" v-else
              >{{ domain.sld }}{{ domain.tld }}</span
            >
            <span
              class="rounded-md px-2 py-1 text-[12px] bg-black text-white ml-2"
              v-if="domain.premium"
              >Premium</span
            >
            <p class="domain font-bold w-full text-sm" v-if="domain.idnName">
              IDN: {{ domain.idnName }}
            </p>
            <div class="description w-full flex-1 text-sm text-gray-300" v-if="domain.description">
              {{ domain.description }}
            </div>
            <div
              class="description w-full flex-1 text-sm text-gray-500"
              v-if="!domain.avaliable & (domain.status != 'inCheck')"
            >
              Rất tiết, tên miền đã có người mua
            </div>
            <div class="description w-full flex-1 text-sm text-gray-300" v-else-if="domain.inCart">
              Tên miền đã thêm vào giỏ hàng.
            </div>
          </div>
        </div>
        <div class="pricing flex items-center">
          ...
          <div class="price ml-5 text-right" v-if="domain.avaliable">
            <div
              class="h-found-domain-cards-item__old-price text-sm text-gray line-through"
              v-if="domain.before > 1"
            >
              {{ $currency(domain.before) }}
            </div>
            <div
              class="h-found-domain-cards-item__discount mob-tab font-bold text-2xl text-red-500"
            >
              {{ $currency(domain.register) }}
            </div>
            <div
              class="h-found-domain-cards-item__cycle font-light text-gray text-sm flex justify-end text-left"
            >
              {{ domain.period }} năm
              <Tooltip bg="light" size="lg" position="bottom" class="cursor-pointer">
                ...
                <template v-slot:content>
                  <p v-if="domain.feeRegOrigin">
                    Lệ phí đăng ký: {{ $currency(domain.feeRegOrigin) }}
                  </p>
                  <p v-if="domain.feeRenOrigin">
                    Phí duy trì: {{ $currency(domain.feeRenOrigin) }}
                  </p>
                  <p v-if="domain.feeManOrigin">
                    Phí dịch vụ quản trị: {{ $currency(domain.feeManOrigin) }}
                  </p>
        ...
        <div class="actions min-w-[300px] flex justify-end">
          <div v-if="domain.status == 'inCheck'">
            <Button
              btnClass="bg-white text-gray"
              icon="heroicons-outline:search"
              isLoading
              textLoading="Đang kiểm tra..."
            ></Button>
          </div>
          <div v-else-if="domain.avaliable === false">
            <Button
              btnClass="bg-gray-50 text-gray"
              icon="heroicons-outline:user"
              @click.stop="getWhoisInfo(domain)"
              text="Xem whois"
            />
          </div>
          <div v-else-if="domain.inCart">
            <Button
              @click="handlePayCart()"
              btnClass="bg-red-50 text-red-500"
          ...
          <div v-else>
            <Button
              ...
              :btnClass="
                domain.isFeatured ? 'bg-primary text-white' : 'text-primary border-2 border-primary'
              "
          ...
          <div v-if="domain.inCart" class="text-right">
            <Button
              @click="removeInCart(domain)"
              btnClass=" text-gray-500 btn-sm"
              icon="mdi:times"
            />
          </div>
        </div>
        ...
        <div class="flex">
          ...
          <div class="ml-3">
            <h3 class="text-sm font-medium text-gray-800">
              Bạn đừng lo, hãy xem các tên miền mà chúng tôi tìm thấy cho bạn bên dưới
            </h3>
          </div>
        </div>
      </div>
      <div class="rounded-md bg-blue-50 p-4" v-if="domain.avaliable & !domain.inCart">
        <div class="flex">
          ...
          <div class="ml-3">
            <h3 class="text-sm font-medium text-gray-800">
              Xin chúc mừng, chúng tôi đã tìm thấy tên miền phù hợp nhu cầu của bạn.
            </h3>
          </div>
        </div>
      </div>
  ...
  <Heading
    text="Tên miền được tìm thấy"
    class="text-primary uppercase text-lg border-b-2 border-gray-50 pb-2 mb-2 mt-10"
  />
  <div v-for="domain in domains" :key="domain.id">
    <div
      v-if="!domain.error && !domain.isFeatured"
      class="rounded-md px-6 bg-white mb-2 py-4"
      :class="[
        domain.status == 'inCheck'
          ? 'check-domain'
          : domain.status == 'ok'
      ...
      <div class="flex items-center flex-wrap py-2">
        <div class="name flex-1 flex items-center">
          ...
          <div class="">
            ...
            <div
              class="description w-full flex-1 text-sm text-gray-500"
              v-else-if="domain.avaliable & !domain.inCart"
            >
              Chúc mừng, bạn có thể đăng ký tên miền này.
            </div>
            <div class="description w-full flex-1 text-sm text-gray-500" v-else-if="domain.inCart">
              Tên miền đã thêm vào giỏ hàng.
            </div>
          </div>
        </div>
    ...
    <div class="p-5 flex space-x-4 relative">
      ...
      <div class="">
        <div v-if="loadingWhois" class="w-full flex justify-center bg-white p-5">
          <svg
            class="animate-spin -ml-1 mr-3 h-6 w-6 text-gray-500"
        ...
        <div class="text-sm mb-10" v-if="whoisInfo.whois && loadingWhois == false">
          <div class="space-y-2">
            <ul class="leading-7">
              ...
              <li>
                <b>Nameservers: </b>
                <p v-for="(nameserver, index) in whoisInfo?.whois.nameservers" :key="index">
                  {{ nameserver }}
                </p>
              </li>
              <li>
