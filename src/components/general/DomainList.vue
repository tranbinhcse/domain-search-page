<template>
  <Table :headings="['', 'Tên miền', 'Ngày hết hạn', 'Gia hạn', 'Trạng thái', 'Thao tác']"
    ...
    <table-row v-for="domain in domains" :key="domain.id">
      ...
      <table-col>
        <badge :variant="domain.status" :labels="{ Active: 'Hoạt động', inactive: 'Đã huỷ', Pending: 'Đang xử lý' }" />
      </table-col>
      <table-col>
        <action-button @click="router.push({name: 'domainDetail', params: { domainId: domain.id, domain: domain.name}})" class="bg-gray-50">
