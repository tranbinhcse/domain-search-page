<template>
  ...
  <div v-show="open" role="alert" class="fixed inset-0 left-auto bottom-auto top-[72px] right-4 z-40">
    <div v-for="({type, message, id}) in notifications" :key="id" class="notification-item mb-3">
      <div class="bg-white dark:bg-slate-600  drop-shadow-md  inline-flex min-w-80 px-4 py-3 rounded-md text-sm border" :class="typeColor(type)">
        <div class="flex w-full justify-between items-start">
          <div class="flex">
            ...
            <div>
              <div  v-if="type === 'warning'" class="text-yellow-800 dark:text-yellow-100">{{ ('Warning') }}</div>
              <div  v-if="type === 'error'" class="text-red-800 dark:text-red-100">{{ 'Error' }}</div>
              <div  v-if="type === 'success'" class="text-primary dark:text-primary ">{{ 'Success' }}</div>
              <div>
                {{ message }}
              </div> 
            </div>
