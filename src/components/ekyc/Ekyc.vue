<template>
  ...
  <div
    v-if="step === 2"
    key=""
    class="mx-auto max-w-2xl px-4 pb-24 pt-16 sm:px-6 lg:max-w-7xl lg:px-8 bg-white dark:bg-slate-700 dark:text-slate-300 rounded-md"
  >
    <p>Xác nhận thông tin</p>
    <p>Bạn cần kiểm tra để đảm bảo thông tin bên dưới là chính xác:</p>
    <form
      @submit.prevent="handleSubmitForm()"
      class="mt-10 space-y-8 border-b border-gray-900/10 pb-12 sm:space-y-0 sm:divide-y sm:divide-gray-900/10 dark:sm:divide-slate-600/40 sm:border-t sm:pb-0"
    >
...
.group_cmnd .cmnd_1:before {
  background:
    url(@/assets/ekyc/CCCD_Small.svg) center no-repeat,
    #f6f6f6;
...
.group_cmnd .cmnd.er:after {
  background:
    url(@/assets/ekyc/Failer_ico.svg) center no-repeat,
    #fff;
...
.group_cmnd .cmnd.ok:after {
  background:
    url(@/assets/ekyc/Success_ico.svg) center no-repeat,
    #fff;
  background-size: contain;
  border-radius: 50%;
}
