...
function loadFaceAPI() {
  if (hasGetUserMedia()) {
    ...
    $('.ePopup').fadeIn()
    $('#ErrorNdung').html(
      `<p style='margin-bottom:16px;'><span style='color:#FC4649;'>Hãy bật Camera</span></p>`
    )
    $('#cLoading').fadeOut()
  }
}
...
function enableCam(event) {
  if (!faceLandmarker) {
    $('.ePopup').fadeIn()
    $('#ErrorNdung').html(
      `<p style='margin-bottom:16px;'><span style='color:#FC4649;'>Camera đang bật, xin chờ giây lát!</span></p>`
    )
    $('#cLoading').fadeOut()
    return
  }
...
let indexAction = 0
var arrAction = [
  { name: 'right', description: 'Xoay phải' },
  { name: 'up', description: 'Nhìn lên' },
  { name: 'down', description: 'Nhìn xuống' },
  { name: 'left', description: 'Xoay trái' },
  { name: 'straight', description: 'Nhìn thẳng' }
]
    ...
    success: function (data) {
      ...
      if ((response.verify_result == 2 || response.verify_result == 1) && response.sim > 0.6) {
        ...
        document.getElementById('result').value =
          verifyResults[response.verify_result_text.toLowerCase().replace(' ', '_')]
        document.getElementById('sim').value = Number((response.sim * 100).toFixed(2)) + '%'
        document.getElementById('wearing_mask').value =
          response.wearing_mask == 'NO' ? 'Không đeo khẩu trang' : 'Có đeo khẩu trang'
        document.getElementById('face_anti_spoof_status').value =
          response.face_anti_spoof_status.status == 'REAL' ? 'Ảnh mặt thật' : 'Không xác định'
        ...
        document.getElementById('face_fail').innerHTML =
          'Nhận diện khuôn mặt không trùng khớp với CMND/CCCD'
        ...
        if (blob.size >= 100352) {
          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              if (response.errorcode != undefined) {
                ...
                if (hientai - namsinh < 18 || hientai - namsinh > 23) {
                  ...
                  $('.ePopup').fadeIn()
                  $('#ErrorNdung')
                    .html(`<p style='margin-bottom:16px;'><span style='color:#FC4649;'>Rất tiếc! Bạn <b>không thuộc đối tượng nhận tên miền</b> miễn&nbsp;phí.</span></p>
                  ...
                  $('#btnWhoisDomain').click(function () {
                    ...
                    let dataDomain = replaceVar($('#txtSearch').val().split('.')[0]).replace(
                      /[^a-zA-Z0-9.]/g,
                      ''
                    )
                    let hreftenmien = dataDomain + giaTri
                    location.href =
                      'https://www.matbao.net/ten-mien/ket-qua-kiem-tra-ten-mien.html?tenmien=' +
                      hreftenmien +
                      '#boxwhoisincheckdomain'
                  })
                  $('#cLoading').hide()
                  isok = 1
                } else {
                  document.getElementById('front_is_full').value =
                    response.is_full == 1 ? 'Giấy tờ đầy đủ' : ''
                  document.getElementById('front_id_check').value =
                    response.id_check == 'REAL' ? 'Giấy tờ thật' : ''
                  document.getElementById('front_id_logic_message').value =
                    response.id_logic_message == 'OK'
                      ? 'Không có lỗi logic'
                      : response.id_logic_message
            ...
            error: function (_data) {
              ...
              $('.mattruoc').addClass('input_err')
              $('#ErrorNdung').html(
                `<p style='margin-bottom:16px;'><span style='color:#FC4649;'>Đã có lỗi xẩy ra, xin thử lại!</span></p>`
              )
              $('#cLoading').hide()
              $('#xacthuc_1').removeClass('enabled')
              cmt1.value = null
          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              if (response.errorcode != undefined) {
                ...
                $('.vmatsau').addClass('ok')
                document.getElementById('back_is_full').value =
                  response.is_full == 1 ? 'Giấy tờ đầy đủ' : ''
                document.getElementById('back_id_check').value =
                  response.id_check == 'REAL' ? 'Giấy tờ thật' : ''
                document.getElementById('back_id_logic_message').value =
                  response.id_logic_message == 'OK'
                    ? 'Không có lỗi logic'
                    : response.id_logic_message

                document.getElementById('issue_by').value =
                  response.issue_by != 'N/A' ? response.issue_by : ''
                document.getElementById('issue_date').value =
                  response.issue_date != 'N/A' ? response.issue_date : ''
                document.getElementById('ethnicity').value =
                  response.ethnicity != 'N/A' ? response.ethnicity : ''
                document.getElementById('religion').value =
                  response.religion != 'N/A' ? response.religion : ''
...
eventInfoUpdate = function () {
  ...
  let _arrdiachi =
    ...
    '☂' +
    $('#selectCountry option:selected').attr('data-code') +
    '|'
  _arrdiachi +=
    $('#selectState option:selected').text() +
    '☂' +
    $('#selectState option:selected').attr('data-code') +
    '|'
  _arrdiachi +=
    $('#selectDistrict option:selected').text() +
    '☂' +
    $('#selectDistrict option:selected').attr('data-code') +
    '|'
  _arrdiachi +=
    $('#selectWard option:selected').text() +
    '☂' +
    $('#selectWard option:selected').attr('data-code') +
    '|'
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.Status) {
        ...
        $('.ePopup').fadeIn()
        $('#ErrorNdung').html(
          `<p style='margin-bottom:16px;'><span style='color:#FC4649;'>${data.Message}</span></p>`
        )
        $('#cLoading').fadeOut()
      }
    }
