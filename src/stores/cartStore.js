...
export const useCartStore = defineStore('cartStore', () => {
  ...
  async function getQuote() {
    ...
    invalidIndexes.forEach((index) => {
      ...
      if (cartItems.value[index].type == 'domain' && quoteItems[index].valid) {
        cartItems.value[index].period =
          quoteItems[index].domains[cartItems.value[index].name].period
        ...
        cartItems.value[index].recurring_price =
          quoteItems[index].domains[cartItems.value[index].name].recurring_price
        cartItems.value[index].setup = quoteItems[index].domains[cartItems.value[index].name].setup
        cartItems.value[index].nameservers =
          quoteItems[index].domains[cartItems.value[index].name].nameservers
        ...
        const cycles = computed(() => {
          return quoteItems[index].domains[cartItems.value[index]?.name]?.config?.product.find(
  ...
  async function addToCart(newItem) {
    ...
    if (newItem.itemType == 'product') {
      const exists = cartItems.value.some(
        (existingProduct) =>
          existingProduct.product_id === newItem.product_id &&
          existingProduct.domain === newItem.domain
      )
      if (!exists) {
        const productItem = {
          type: 'product',
  ...
  async function updateItem(domain) {
    ...
    const index = cartItems.value.findIndex(
      (existingDomain) => existingDomain.name === domain.name && existingDomain.tld === domain.tld
    )
    console.log(domain)
  ...
  async function order(router) {
    ...
    for (const item of cartItems.value) {
      ...
      if (item.type === 'domain') {
        ...
        updatedCartItems.push({
          ...item,
          nameservers: ['ns1.tino.vn', 'ns2.tino.vn'],
