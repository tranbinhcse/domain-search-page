...
export async function resizeImage(imageDataUrl) {
  let parsedData = await loadImage(imageDataUrl, {
    maxWidth: import.meta.env.VUE_APP_IMAGE_MAX_WIDTH,
    maxHeight: import.meta.env.VUE_APP_IMAGE_MAX_HEIGHT,
    orientation: true,
    canvas: true,
    meta: true
  })
...
// Crop
export async function cropImage(imageDataUrl, width, height, cropWidth, cropHeight) {
  let parsedData = await loadImage(imageDataUrl, {
    maxWidth: import.meta.env.VUE_APP_IMAGE_MAX_WIDTH,
    maxHeight: import.meta.env.VUE_APP_IMAGE_MAX_HEIGHT,
...
export function cropAndNormalize(canvas, longEdge, x1, y1, x2, y2) {
  ...
  normCanvas
    .getContext('2d')
    .drawImage(canvas, x1, y1, x2 - x1, y2 - y1, 0, 0, normCanvas.width, normCanvas.height)
