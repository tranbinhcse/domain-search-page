...
const idLogicErrorMessage = {
  ...
  'Expiry subtract birthday not good': 'Ngày tháng năm sinh và ngày tháng hết hạn không hợp lệ.',
  'ID can be fake': 'Giấy tờ có thể đang bị giả mạo.'
}

export function convertAddressOcrData(ocrData) {
  console.log(ocrData)
...
export function ocrFrontCardChecking(ocrData) {
  console.log(ocrData)
  if (ocrData) {
    if (ocrData.result_code === 200) {
      if (ocrData.is_full !== 1 && import.meta.env.VUE_APP_CHECK_FULL === '1') {
      ...
      return { success: true, message: 'Phân tích ảnh mặt trước hợp lệ.' }
    }
  } else {
    return {
      success: false,
  ...
  if (ocrData.result_code === 501) {
    return {
      success: false,
      message: 'License chưa được kích hoạt. Liên hệ với VNNIC để kích hoạt license.'
    }
  }
...
export function ocrBackCardChecking(backCardOcrData, frontCardOcrData) {
  if (backCardOcrData) {
    if (backCardOcrData.result_code === 200) {
      if (backCardOcrData.is_full !== 1 && import.meta.env.VUE_APP_CHECK_FULL === '1') {
        return { success: false, message: 'Hình ảnh CMT không đầy đủ.' }
      }
...
export function faceVerificationChecking(faceVerificationData) {
